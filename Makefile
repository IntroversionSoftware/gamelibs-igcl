...
CXXFLAGS ?= -O2
CXXFLAGS += -Iincludes -I$(includedir)

HEADERS = \
	includes/*.h \

SOURCES = \
	wrapper/cApiWrapper.cpp \

SOURCES := $(wildcard $(SOURCES))
HEADERS := $(wildcard $(HEADERS))

HEADERS_INST := $(patsubst includes/%,$(includedir)/%,$(HEADERS))
...
$(OBJ_DIR)/.cflags: .force-cflags | $$(@D)/.
    ...
    if test x"$$FLAGS" != x"`cat $(OBJ_DIR)/.cflags 2>/dev/null`" ; then \
        echo "    * rebuilding igcl: new build flags or prefix"; \
